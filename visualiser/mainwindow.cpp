...
void MainWindow::onStep() {
    ...
    for (int i = 0; i < stepsThisTick; ++i) {
        ...
        if (dist_u > dist[u]) continue; // Skip stale entries
        ...
        for (int eidx : graph.adj[u]) {
            ...
            if (nd < dist[v]) {
                ...
                // Visual feedback: Draw the discovery edge
                if (!e.lineItem) {
...
void MainWindow::onStart() { 
    ...
    if (!dijkstra_initialized) initDijkstra();
    running = true; 
    timer->start(timerInterval()); 
}
...
void MainWindow::reconstructPath() {
    ...
    while (cur != -1 && parent[cur] != -1) {
        int p = parent[cur];
        // Find the edge between p and cur to highlight it
