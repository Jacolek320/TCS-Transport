...
void load_osm_pbf(const QString& path, Graph& graph) {
    ...
    // 2. Calculate the 30% central window
    ...
    double half_w = (total_bbox.top_right().lon() - total_bbox.bottom_left().lon()) * 0.30 / 2.0;
    double half_h = (total_bbox.top_right().lat() - total_bbox.bottom_left().lat()) * 0.30 / 2.0;
    ...
    // 3. First pass: Only store node locations if they are inside the 30% box
