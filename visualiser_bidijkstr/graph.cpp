...
const double percent = 1.0;         //what part of central window will be used, at most 1.0
...
void load_osm_pbf(const QString& path, Graph& graph) {
    ...
    double half_w = (total_bbox.top_right().lon() - total_bbox.bottom_left().lon()) * percent / 2.0;
    double half_h = (total_bbox.top_right().lat() - total_bbox.bottom_left().lat()) * percent / 2.0;
