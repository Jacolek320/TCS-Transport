...
void MainWindow::onLoad() {
    ...
    bidijkstra.graph.clear();
    bidijkstra.graph.reserve(100000, 200000);
    scene->clear();
    ...
    load_osm_pbf(f, bidijkstra.graph);
    vt.compute(bidijkstra.graph);

    for (size_t i = 0; i < bidijkstra.graph.nodes.size(); ++i) {
        QPointF p = vt.toScene(bidijkstra.graph.nodes[i].x, bidijkstra.graph.nodes[i].y);
...
void MainWindow::onStep() {
    ...
    std::vector<int> usedNow = bidijkstra.doSteps(stepsThisTick);
    ...
    for(int eidx : usedNow) {
        auto &e = bidijkstra.graph.edges[eidx];
        int u = e.u_idx;
        colorNodeByDistance(u);
    }

    if(bidijkstra.isFinished()) {
        reconstructPath();
        onPause();
        return;
    }
...
void MainWindow::updateNodeVisual(int idx, QColor color, bool isBig) {
    ...
    QPointF p = vt.toScene(bidijkstra.graph.nodes[idx].x, bidijkstra.graph.nodes[idx].y);
...
void MainWindow::initAlg() {
    bidijkstra.set(*start_idx, *end_idx);
    initialized = true;
}

void MainWindow::colorNodeByDistance(int idx) {
    double t = std::min(1.0, bidijkstra.getDist(idx) / 5000.0);
    QColor c((1.0-t)*255, 0, t*255);
    nodesItems[idx]->setBrush(QBrush(c));
}

void MainWindow::reconstructPath() {
    std::vector<int> pathFound = bidijkstra.reconstruct();

    for(int eidx : pathFound) {
        auto &e = bidijkstra.graph.edges[eidx];
        int u = e.u_idx;
        int v = e.v_idx;
        QPointF p1 = vt.toScene(bidijkstra.graph.nodes[u].x, bidijkstra.graph.nodes[u].y);
        QPointF p2 = vt.toScene(bidijkstra.graph.nodes[v].x, bidijkstra.graph.nodes[v].y);
        auto *line = scene->addLine(QLineF(p1, p2), QPen(Qt::yellow, 4.0));
        line->setZValue(3);       //Bring to front
        edgesItems.push_back(line);
        nodesItems[bidijkstra.graph.edges[eidx].v_idx]->setBrush(QBrush(Qt::yellow));
    }
}